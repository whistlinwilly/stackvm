...
var smmTest = TestCase{
	...
	Prog: MustAssemble(
		...
		":chooseNext", "fnz", // &$X i : retIp   -- fork next if i < 9
		"dup", 2, "swap", "storeTo", // $X=i : retIp
		"dup", 4, "mul", 0x0100, "add", // $X &used[$X] : retIp
		"dup", "fetch", // $X &used[$X] used[$X] : retIp
		2, "hnz", // $X &used[$X] : retIp
		1, "store", // $X : retIp -- used[$X] = 1
		"ret", // $X :
